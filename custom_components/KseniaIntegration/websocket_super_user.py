...
class WebsocketSuperUser:
    ...
    def __init__(
        ...
        macAddr,
        pinSuper,
        coordinator: AlarmDataCoordinator,
        websocket: SimpleAlarmWebSocketClient,
    ):
    ...
    async def connectSuperUser(self):
            ...
            try:
                await self.send(
                    addCRC(
                        ...
                        + self._mac
                        + '", "CMD":"LOGIN", "ID": "65535", "PAYLOAD_TYPE":"IP_SUPERV", "PAYLOAD":{ "PIN": "'
                        + self._pinSuper
                        + '"}, "TIMESTAMP":"'
                        + str(int(time.time()))
                        + '", "CRC_16":"0x0000"}'
        ...
        await self.send(
            addCRC(
                '{"SENDER":"012345678901", "RECEIVER":"'
                + self._mac
                + '", "CMD":"REALTIME","ID":"2","PAYLOAD_TYPE":"REGISTER","PAYLOAD":{ "ID_LOGIN": "'
                + str(self._id)
                + '" ,  "TYPES":["STATUS_PARTITIONS", "STATUS_ZONES", "ZONES"] },"TIMESTAMP" : "'
                + str(int(time.time()))
                + '","CRC_16":""}'
            )
        )
